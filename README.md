...
├── style.css                # Layout & komponen UI
├── app.js                   # Logika klien
├── .gitignore               # Proteksi file rahasia dari GitHub
├── eventra_import.sql       # File import database
└── api/
    ├── config.example.php   # TEMPLATE config — rename jadi config.php
    ├── config.php           # ⚠️ Config asli — TIDAK ter-upload ke GitHub
    ├── koneksi.php          # Koneksi database (baca dari config.php)
    ├── login.php            # POST — autentikasi admin
    ├── ganti_password.php   # POST — ganti password admin
    ├── produk.php           # GET/POST/PUT/DELETE — CRUD produk
    ├── pesanan.php          # GET/POST/PUT/DELETE — CRUD pesanan
    └── .htaccess            # Blokir akses langsung ke config & koneksi
```
...
**1. Clone dari GitHub atau salin folder**
```
...
```
Rename: api/config.example.php  →  api/config.php
Lalu buka api/config.php dan sesuaikan isinya
```
...
→ Tab Import → Pilih eventra_import.sql → Go
```
...
## 🔑 Cara Ganti Password di phpMyAdmin

1. Buka `http://localhost/phpmyadmin`
2. Klik database `eventra` → tabel `admin`
3. Klik **Ubah** (ikon pensil) di baris admin
4. Kolom `password` → ketik password baru langsung
5. Pastikan kolom **Fungsi** di sebelahnya **KOSONG**
6. Klik **Simpan** ✅

---

## 🔒 Upload ke GitHub (Aman)

File `api/config.php` sudah masuk `.gitignore` — **tidak akan ter-upload**.
Yang ter-upload ke GitHub hanya `api/config.example.php` sebagai template.
...
Orang lain yang clone repo Anda cukup rename `config.example.php` → `config.php` lalu isi dengan data mereka sendiri.
